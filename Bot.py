...
async def on_ready():
    ...
    print(bot.user.id)
    print('--------------------')
    await bot.change_presence(game=discord.Game(name='Create By BociL',type=0))
...
async def mute(ctx,target:discord.Member):
    if ctx.message.author.id==(dabs):
      role=discord.utils.get(ctx.message.server.roles,name='nadeko-mute')
...
bot.run('NTU3MTc4MTYyMzI1NDg3NjM3.D3H57A.BXmNIrqYjRKc77DW4Jc4LpS-JAY')
